...
module.exports = {
  get: (url = '', options) => {
    ...
    } else if (url.includes(`${MICROCONTROLLERS_MS}/pictures`)) {
      return Promise.resolve({
        data: require('../microcontrollers.json').filter(micro => micro.measure === 'pictures')
      })
    ...
    } else if (url.includes('/pictures')) {
      return Promise.resolve({
        data: require('../picture.json')
      })
    }
  },
